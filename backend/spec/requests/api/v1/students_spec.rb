...
RSpec.describe 'Api::V1::Students', type: :request do
  ...
  describe 'PATCH /api/v1/students/:id' do
    ...
    it 'returns the updated student data' do
      patch "/api/v1/students/#{student.id}",
            params: { student: { first_name: 'Updated', email: 'new@example.com' } },
            headers: auth_headers(clerk)
      expect(response).to have_http_status(:ok)
      body = JSON.parse(response.body)
      expect(body['success']).to be true
      expect(body['data']['first_name']).to eq('Updated')
    ...
    it 'forbids instructor role from updating' do

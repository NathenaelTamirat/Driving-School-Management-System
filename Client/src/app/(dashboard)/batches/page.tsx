    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold text-blue-600">{stats.submitted}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Approved</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-emerald-600">{stats.approved}</div></CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
                ...
                <tbody>
                  {filtered.map((b) => (
                    <tr key={b.id} className="border-b last:border-0">
                      ...
                      <td className="p-3">
                        <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${statusStyles[b.status] || "bg-gray-100"}`}>
                          {b.status}
                        </span>
                      </td>
        ...
        <div>
          <Card>
            <CardHeader><CardTitle>{selected ? selected.name : "Batch Details"}</CardTitle></CardHeader>
            <CardContent>
              {!selected ? (
